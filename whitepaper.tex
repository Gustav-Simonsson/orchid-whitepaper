...
\begin{abstract}
  ...
  \item An efficient security-hardened auction mechanism suited for
    ...
    bids as part of an attack.
  \item An implementation of stochastic micropayments on the Ethereum
    platform.
  \item A commodity specification for the sale of P2P bandwidth.
...
TODO: Professor Boneh should be consulted before this is finalized. I
vote for Equihash, as it appears hardest to ASIC.
...
a given Econ $e$ are defined to be $\min_{f \in A} dist(t, f)$, for
each of the 256 target addresses $t \in \{1, 2, 4, .. 2^{255}\}$.
...
Econs on the Agora must change keys every 10 Ethereum blocks.
