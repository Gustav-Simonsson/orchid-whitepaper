...
\usepackage[backend=biber,style=authoryear-icomp,url=true]{biblatex}
...
\item Achieving faster downloads on BitTorrent by pretending to be multiple leachers [\citation{freeridingBittorrent}].
\end{itemize}
...
\begin{itemize}
\item Segmenting the Bitcoin mining P2P network, allowing for so-called “51\% attacks” when the attacker controls substantially less than 51\% of the compute power[\citation{bitcoinEclipse}].
\item Removing a file from the BitTorrent DHT by taking over the address space associated with its magnet link[\citation{bittorrentSybilAttacks}].
\end{itemize}
...
\begin{itemize}
\item Inferring medical illnesses, family income, and investment choices of end- users from SSL encrypted web traffic[\citation{broadInferenceAttacks}].
\item Deanonymizing Tor, I2P and \Orchid traffic from global traffic logs[\citation{mixTrafficAnalysis}].
\item Learning the private key of an OpenSSL server through timing analysis[\citation{opensslTimingAttack}].
\end{itemize}
...
\begin{itemize}
\item Targeted DoS attacks used in concert with sybil-attack based monitoring to deanonymize Tor traffic[\citation{DOSvsSec}].
\item DoS off lining for complete control of I2P’s floodfill database, requiring only 20 sybil nodes, thereby deanonyimizing all traffic on the network[\citation{I2P-vigna}].
\end{itemize}
...
Tor \citation{TOR} is a free software project famous for introducing the
...
Lottery ticket type payments are not novel to \Orchid. For interested readers, we recommend the extremely detailed and helpful \citation{DAM}, which includes discussion of extending tickets to support anonymous payments.
...
(\citation{ssl-downgrade}). To perform this attack, the attacker simply
...
\citation{CHORD}. For our purposes, it is enough to note that the following
...
(\citation{peekaboo}). To address this, users accessing the \Orchid{} Network
